...
%>
	...
	<main id="main" data-aos="fade" data-aos-delay="1500">
		...
		<div class="container">
			<h1 class="mb-5">유저 프로필</h1>
			<div class="bg-white shadow rounded-lg d-block d-sm-flex">
				<div class="profile-tab-nav border-right">
					<div class="p-4">
						<div class="img-circle text-center mb-3">
							<img src="../resources/img/user.png" alt="Image" class="shadow">
						</div>
						<h4 class="text-center" id="showNick1"></h4>
					</div>
					<div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
						...
						<a class="nav-link" id="requestBBS" data-toggle="pill" href="#application" role="tab" aria-controls="application" aria-selected="false">
							<i class="fa fa-tv text-center mr-1"></i> 
							내가 쓴 게시글
						</a>
						<a class="nav-link" id="requestReply" data-toggle="pill" href="#application1" role="tab" aria-controls="application1" aria-selected="false">
							<i class="fa fa-tv text-center mr-1"></i> 
							내가 쓴 댓글
						</a>
				...
				<div class="tab-content p-4 p-md-5" id="v-pills-tabContent">
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
								  	<label>Name</label>
								  	<input id="showName" type="text" class="form-control" value="">
								</div>
							</div>
							...
							<div class="col-md-6">
								<div class="form-group">
								  	...
								  	<input id="showNick2" type="text" class="form-control" value="">
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
								  	...
								  	<input id="showID" type="text" class="form-control" value="">
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
								  	<label>Phone number</label>
								  	<input id="showPhone" type="text" class="form-control" value="">
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
								  	<label>Gender</label>
								  	<input id="showGen" type="text" class="form-control" value="" disabled>
								</div>
							</div>
							...
							<div class="col-md-12">
								<div class="form-group">
								  	<label>Bio</label>
									<textarea class="form-control" rows="4" placeholder="자기 자신을 소개해 보세요!"></textarea>
								</div>
							</div>
						</div>
						<div>
					...
					<div class="tab-pane fade" id="application" role="tabpanel" aria-labelledby="application-tab">
						<h3 class="mb-4">내가 쓴 게시글</h3>
						<div class="row">
							<div id="showArea" class="col-md-6">
							</div>
						</div>
					</div>
					<div class="tab-pane fade" id="application1" role="tabpanel" aria-labelledby="application-tab">
						<h3 class="mb-4">내가 쓴 댓글</h3>
						<div class="row">
							<div id="showArea1" class="col-md-6">
							</div>
						</div>
					</div>
	...
	var user = '<%=String.valueOf(session.getAttribute("userid"))%>'
						$.ajax({
							...
							success : function(reciveData) {
								$("#showID").attr('value', reciveData.userid)
								$("#showName").attr('value', reciveData.name)
								$("#showNick1").html(reciveData.nickname);
								$("#showNick2").attr('value', reciveData.nickname)
								$("#showGen").attr('value', reciveData.gender)
								$("#showPhone").attr('value', reciveData.tel)
							},
							error : function(e) {
								alert("error");
								console.log(e);
...
$("#requestBBS").on("click",function(){
	var user = '<%=String.valueOf(session.getAttribute("member"))%>'
	console.log("게시글 클릭됨");
	$("#showArea").empty();
	$.ajax({
		url : "userBbs",
		...
		success : function(reciveBBS){
			$("#showArea").append('<div id="userBBSArea"><table border="1" style="text-align: center; width:950px;"  id="userBBS"><tr style="font-weight: BOLD;"><td>제목</td><td>지역</td><td>작성시간</td></tr></table></div>');
			for(var i = 0 ; i < reciveBBS.length ; i++){
				 $("#userBBS").append(
...
$("#requestReply").on("click",function(){
	...
	$.ajax({
		...
		success : function(reciveReply){
			$("#showArea1").append(' <div id="userReplyArea"> <table border="1" id="userReply" style="text-align: center; width: 950px;"> <tr style="font-weight: BOLD;"><td>게시판 ID</td><td>댓글내용</td><td>작성시간</td></tr></table> </div> ');
