...
public class FlightChangeService {
	...
	public ArrayList<Integer> pricechange(FlightVO vo) {
		...
		try {
			...
			/*
			 * int j=1; //해당 주 만큼 반복 for (int i=0; i<7; i++) {
			 * 
			 * if((days+j) >7) { week = week+1; days= 1; j = 0; }
			 * 
			 * element = driver.findElement(By.xpath(
			 * "/html/body/c-wiz[2]/div/div[2]/c-wiz/div[1]/c-wiz/div[2]/div[1]/div/div[2]/div[2]/div/div/div[2]/div/div[2]/div[2]/div/div/div[1]/div/div/div["
			 * +airmonth2+"]/div[3]/div["+week+"]/div["+j+"]/div/div[2]")); String prices =
			 * element.getText(); if(prices.equals("")) prices = prices.replace("₩", "");
			 * //필터링들 prices = prices.replace(".", ""); prices = prices.replace("만", "00");
			 * if (prices.length() == 4) { prices= prices + "00"; } if (prices.equals("")) {
			 * //값이 비어있으면 0으로 지정 prices= "0"; } int lowprices = Integer.parseInt(prices);
			 * lowest.add(lowprices); j = j+1;
			 * 
			 * }
			 */
